...
public class QuoteProcessorTest {
    ...
    @Test
    public void should_return_same_quotes_when_no_duplicated_emails_sorted() throws Exception {
        // Given
        Quote quote1 = new Quote(now(), "test@test.fr", "axa");
        Quote quote2 = new Quote(now(), "test1@test.fr", "axa");
        Quote quote3 = new Quote(now(), "test2@test.fr", "axa");
        Iterable<Quote> quoteIterable = asList(quote1, quote3, quote2);

        // When
        Set<Quote> quotes = quoteService.getQuotesWithDistinctEmailSortedInReversAlpha(quoteIterable);
    ...
    @Test
    public void should_return_deduplicated_quotes_by_emails_sorted() throws Exception {
        // Given
        Quote quote = new Quote(now(), "test@test.fr", "axa");
        Quote quoteDuplicate = new Quote(now(), "test@test.fr", "amaguiz");
        Quote quote3 = new Quote(now(), "test2@test.fr", "axa");
        Iterable<Quote> quoteIterable = asList(quote3, quote, quoteDuplicate);
